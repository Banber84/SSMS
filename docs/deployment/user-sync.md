...
```bash
sudo ssmsctl node join NodeC 192.168.1.130 nodec1
# 原脚本：sudo scripts/join_node.sh NodeC 192.168.1.130 nodec1
```
...
```bash
sudo ssmsctl node join NodeC 192.168.1.130 nodec1 --skip-existing-users
# 原脚本：sudo scripts/join_node.sh NodeC 192.168.1.130 nodec1 --skip-existing-users
```
...
```bash
sudo ssmsctl user create joincheck2 --quota-gb 1
# 原脚本：sudo scripts/sync_user.sh joincheck2 --quota-gb 1
```
...
# 原脚本：sudo scripts/sync_delete_user.sh alice --keep-data
```

如果需要保留节点本地 home：

```bash
sudo ssmsctl user delete alice --keep-node-home
# 原脚本：sudo scripts/sync_delete_user.sh alice --keep-node-home
```

如果只补删除节点用户：

```bash
sudo ssmsctl user delete alice --nodes-only
# 原脚本：sudo scripts/sync_delete_user.sh alice --nodes-only
```
...
```bash
sudo ssmsctl user create alice --quota-gb 1 --storage-only
# 原脚本：sudo scripts/sync_user.sh alice --quota-gb 1 --storage-only
```
...
```bash
sudo ssmsctl user create alice --quota-gb 1 --nodes-only
# 原脚本：sudo scripts/sync_user.sh alice --quota-gb 1 --nodes-only
```
