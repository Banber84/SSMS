...
sudo -u alice touch /mnt/ssms-alice/manual-NodeA.txt
...
NodeA 可以在 alice 共享目录中创建 manual-NodeA.txt。
```
...
NodeB 可以看到 NodeA 创建的 manual-NodeA.txt。
...
3. NodeB 能看到 NodeA 创建的 manual-NodeA.txt。
